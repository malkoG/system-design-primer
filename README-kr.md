...
액티브-패시브 방식에서는 활성화된 서버와 대기 상태인 비활성화된 서버 사이에 하트비트를 주고 받습니다. 하트비트를 주고 받는 동안 인터럽트가 발생하면, 비활성화된 서버가 활성화된 서버의 IP 주소를 넘겨받아 서비스를 재시작합니다. 

다운타임의 길이는 비활성화된 서버가 이미 `상시` 대기 상태에서 실행 중인지 혹은 `콜드` 대기 상태에서 시작해야 하는지 여부에 따라 결정됩니다. 오직 활성화된 서버만 트래픽을 처리합니다.

액티브-패시브 페일오버는 마스터-슬레이브 페일오버로 언급되기도 합니다. 

#### 액티브-액티브

액티브-액티브 방식에서는 두 서버가 트래픽을 처리하며, 서버 간에 부하를 분산시킵니다.

서버가 외부망에 노출되어 있을 경우, DNS는 두 서버의 공용 IP를 알아야 합니다. 서버가 내부망에 위치한 경우, 애플리케이션 로직은 두 서버에 대해 알고 있어야 합니다.

액티브-액티브 페일오버는 마스터-마스터 페일오버라고도 합니다.

### 페일오버의 단점

* 페일오버는 추가적인 하드웨어를 필요로 하며, 시스템을 더 복잡하게 만들 수도 있습니다. 
